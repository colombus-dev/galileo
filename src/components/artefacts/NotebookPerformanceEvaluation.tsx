...
export function NotebookPerformanceEvaluation({
  notebook,
  className,
}: NotebookPerformanceEvaluationProps) {
  ...
  const pct =
    typeof value === "number" ? Math.round(clamp01(value) * 1000) / 10 : null;
  ...
  const balanceLabel =
    notebook.context?.data.classBalanceLabel ?? inferredBalance?.label;
  const balanceHint =
    notebook.context?.data.classBalanceHint ?? inferredBalance?.hint;
  ...
  const codeLines = notebook.cells.reduce(
    (acc, c) => acc + getNonEmptyLineCount(c.code),
    0,
  );
  const execSeconds = notebook.context?.codeQuality?.executionTimeSeconds;
  const notebookName = notebook.id;
  const isAccuracy = (metricName ?? "").toLowerCase().includes("accuracy");
  const isImbalanced =
    (balanceLabel ?? "").toLowerCase().includes("dés") ||
    (balanceLabel ?? "").toLowerCase().includes("imbal");
  ...
  const availableRecommended = resolvedRecommended.filter(
    (r) => typeof r.value === "number",
  );
  const missingRecommended = resolvedRecommended.filter(
    (r) => r.value === null,
  );
  return (
    <section
    ...
    >
          ...
          <div className="mt-4 border-t border-slate-100 pt-3 text-xs">
            {isOnTest ? (
              ...
              <div className="flex items-center gap-2 text-amber-700">
                ...
                <span>
                  Évalué sur train set (score potentiellement optimiste)
                </span>
              </div>
            ) : (
        ...
        <div className="rounded-2xl border border-emerald-200 bg-white p-5">
          ...
          {dist.length > 0 && distTotal > 0 ? (
            ...
            <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
              Distribution indisponible (ajoute-la dans le notebook ou les
              artefacts).
            </div>
          )}
        </div>
            ...
            {cm ? (
              ...
              <div className="text-center">
                ...
                <div className="text-xs text-slate-600">
                  Ajoute un artefact de matrice de confusion
                </div>
              </div>
            )}
          </div>
            ...
            <div className="min-w-0">
              ...
              {isOnTrain ? (
                <div className="mt-1 text-xs text-amber-800">
                  Un score sur train est souvent surestimé. Idéalement, évalue
                  sur test set ou via validation.
                </div>
              ) : null}
              {isAccuracy && isImbalanced ? (
                <div className="mt-1 text-xs text-amber-800">
                  Avec des classes déséquilibrées, l’accuracy peut masquer des
                  erreurs. Regarde la matrice de confusion et F1-macro.
                </div>
              ) : null}
            </div>
          </div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2 text-xs">
          ...
          {missingRecommended.length > 0 ? (
            <span className="text-slate-500">
              À calculer dans ce notebook:{" "}
              {missingRecommended.map((m) => m.label).join(", ")}
            </span>
          ) : null}
        </div>
      ) : null}
          ...
          <StatCard
            ...
            value={
              typeof execSeconds === "number"
                ? `~${Math.round(execSeconds)}s`
                : "—"
            }
            valueClassName={
              typeof execSeconds === "number" ? "text-amber-700" : undefined
            }
      ...
      <div className="mt-8">
        ...
        <PatternPipeline notebookName={`${notebookName}`} />
      </div>
    </section>
  );
}
