	...
	useEffect(() => {
		...
		(async () => {
			try {
				...
				if (cancelled) return;
				setError(e instanceof Error ? e.message : "Erreur lors du chargement du notebook");
				setNotebook(null);
			} finally {
				if (cancelled) return;
				setIsLoading(false);
		...
		<div className="w-full max-w-7xl mx-auto px-6 py-10">
			...
			<div className="mt-8">
				{!selectedId ? (
					...
					<div className="rounded-2xl border border-slate-200 bg-white p-5 text-slate-700">Chargement…</div>
				) : error ? (
						...
						<div className="rounded-2xl border border-slate-200 bg-white p-6">
							<div className="flex items-start justify-between gap-6">
								<div>
									...
									<div className="text-sm text-slate-600 mt-1">{notebook.student} - {notebook.title}</div>
								</div>
